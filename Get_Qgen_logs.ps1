...
foreach ($server in $servers){
    ...
    if(Test-Connection -Cn $server -BufferSize 16 -Count 1 -ea 0 -quiet){
        $logs += Get-ChildItem "\\$server\i$\ali\site\log\qgen_$Modality`_*.log" | Sort -Property LastWriteTime | `
...
else{ echo "Could not locate query logs for the modality: $Modality" }
